...
const defaultVersion = Array.from(controlData.match(/default_version = '(.*)'/) ?? [])[1];
let nextVersion = semver.format(semver.increment(semver.parse(defaultVersion), 'patch'));
...
if (args.overwrite === false) {
  try {
    ...
    console.log(`Version ${nextVersion} already exists. Use --overwrite to overwrite.`);
    Deno.exit(1);
  }
  catch (_){
    // do nothing
